...
var GLOBAL_BATCH = make([]Data, PAGE_SIZE)
...
func handleUDPConnection(conn *net.UDPConn) {
	...
	if err != nil {
		...
		GLOBAL_BATCH[GLOBAL_COUNTER].AccX[LOCAL_COUNTER], err = strconv.ParseFloat(data[ACC_X], 64)
		if err != nil {
			fmt.Println(ACC_X)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].AccY[LOCAL_COUNTER], err = strconv.ParseFloat(data[ACC_Y], 64)
		if err != nil {
			fmt.Println(ACC_Y)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].AccZ[LOCAL_COUNTER], err = strconv.ParseFloat(data[ACC_Z], 64)
		if err != nil {
			fmt.Println(ACC_Z)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].GyroX[LOCAL_COUNTER], err = strconv.ParseFloat(data[GYRO_X], 64)
		if err != nil {
			fmt.Println(GYRO_X)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].GyroY[LOCAL_COUNTER], err = strconv.ParseFloat(data[GYRO_Y], 64)
		if err != nil {
			fmt.Println(GYRO_Y)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].GyroZ[LOCAL_COUNTER], err = strconv.ParseFloat(data[GYRO_Z], 64)
		if err != nil {
			fmt.Println(GYRO_Z)
			log.Fatal(err)
			return
		}
		GLOBAL_BATCH[GLOBAL_COUNTER].Time[LOCAL_COUNTER] = time.Now()
...
func initBatch(name string, capacity int) (d *Data){
	d = &Data{}
	d.Name = name
	d.AccX = make([]float64, capacity)
	d.AccY = make([]float64, capacity)
	d.AccZ = make([]float64, capacity)
	d.GyroX = make([]float64, capacity)
	d.GyroY = make([]float64, capacity)
	d.GyroZ = make([]float64, capacity)
	d.Time = make([]time.Time, capacity)
	return
}

func(d *Data) mock() {
	for i := 0; i < len(d.AccX); i++ {
		d.AccX[i] = rand.Float64()
		d.AccY[i] = rand.Float64()
		d.AccZ[i] = rand.Float64()

		d.GyroX[i] = rand.Float64()
		d.GyroY[i] = rand.Float64()
		d.GyroZ[i] = rand.Float64()
	}
}
